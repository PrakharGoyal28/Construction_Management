...
const ReceiveInventory = () => {
  ...
  const getInventory = async () => {
    try {
      const response = await axios.get(`${BASE_URL}/materials/getMaterialByType?type=Recieve`);
      setInventory(response.data.data);
      filterMaterials(response.data.data, 'InTransit');
    } catch (error) {
      console.error(error);
    }
  };
  ...
  const filterMaterials = (data, status) => {
    const filtered = data.filter((item) => item.Recieve === status);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.buttonContainer}>
        {['InTransit', 'Recievable', 'ForApproval', 'Approved'].map((status) => (
          <TouchableOpacity
            key={status}
